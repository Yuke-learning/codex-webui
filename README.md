...
- 线程列表、搜索、详情与流式事件刷新。
- 左侧刷新按钮会立即遍历 `thread/list` 的全部分页，同步所有未归档对话；若当前打开了任务，还会强制刷新其最近 8 轮详情。更早历史按需通过“加载更早的对话”分页取得，避免长对话拖慢切换。
- 最近查看过的 12 个任务只在浏览器内存中保留详情缓存：切回时先立即显示缓存，再后台核对最近页。详情 JSON 会根据浏览器能力使用 Brotli 或 gzip 压缩；对话内容不会写入 localStorage、磁盘或第三方服务。
- 左侧按 Git 项目分组对话，未关联项目的线程单独归类；项目节可随时折叠并保留本机浏览器偏好，即使其包含正在预览的对话。项目标题、悬停文字、折叠箭头、数量徽标、层级轨道、选中指示条和“非项目”虚线分组均使用独立的导航语义变量，避免任何主题继承主内容区颜色后出现同色文字。左侧导航与主消息区独立滚动，顶部操作栏固定在消息区之外。
...
- 用户与 Codex 消息支持 CommonMark 风格 Markdown、表格、链接、KaTeX 行内/块级公式和常见语言代码高亮；可在“设置 → 偏好设置”中开启消息气泡时间，用户消息使用轮次开始时间，Codex 回复使用轮次完成时间。代码块、表格与公式在手机上可独立横向滚动。渲染前后均经过 DOMPurify 清理，禁用远程图片和可执行 HTML，所有依赖与字体均打包到本地，不加载第三方 CDN。
- 新建线程、发送消息/追加指令、停止、改名、归档和带确认的删除。
- 从 app-server 动态读取可用模型与推理强度；详情页显示线程的实际设置，并可将新的模型/强度应用于后续 turn。
- 左侧栏的新建对话按钮上方显示 CC Switch 全局服务商入口，并可在“设置 → 偏好设置”中选择是否在主页显示：服务端只返回服务商 ID、名称和启用状态，不向浏览器传递 API Key 或完整 API URL。代理接管模式在下一轮热切换；普通配置模式等待当前任务结束后切换、重连 Codex 并刷新模型列表，验证失败会自动切回原服务商。
