...
当前 Mac 上的 Codex CLI 已提供实验性 `app-server` 与 `remote-control`。生成的本地协议 schema 包含 `thread/list`、`thread/read`、`thread/turns/list`、`thread/start`、`thread/resume`、`thread/fork`、`thread/archive`、`thread/delete`、`thread/name/set`、`turn/start`、`turn/steer` 与 `turn/interrupt` 等方法，能覆盖本项目核心功能。
...
| `GET /api/threads/:id` | `thread/resume(initialTurnsPage)` | 只获取最近 8 轮完整 turn；不支持分页的旧 CLI 回退到完整读取 |
| `GET /api/threads/:id/turns?cursor=…` | `thread/turns/list` | 按需取得更早的 12 轮 turn |
...
- 手动全量同步：左侧刷新按钮遍历 `thread/list` 的全部游标分页，并在存在当前选中任务时重新读取其最近页；同步过程中禁止重复触发。更早 turn 仅在用户明确请求时分页读取。
- 主题化侧栏：项目标题的普通背景、悬停背景、主文字、次要图标、数量徽标和“非项目”分组分别使用导航语义变量，禁止继承主内容区文字色或直接复用侧栏底色，保证所有主题下均有稳定对比度。
- 线程详情：消息时间线、可选的消息气泡时间、流式输出、工具执行状态、计划与最终答复。气泡时间使用 app-server 提供的轮次开始/完成时间，不根据浏览器接收时间伪造消息时间。
