...
```mermaid
flowchart LR
  Browser["手机 / 浏览器"] -->|"Tailscale 私网 HTTPS"| Web["Web UI + Gateway\nNext.js / TypeScript"]
...
Gateway 不应连接一个公开的 app-server 监听端口，而应在本机启动子进程：
...
- **前后端**：Next.js + TypeScript，Route Handlers 作为 Gateway，React 负责移动端界面。
...
- **部署**：Mac 用户级 `launchd` 守护 Gateway 与 app-server；Tailscale 提供私网访问。
